  ...
  const calculateTotalBalance = (ass) => {
    ...
    for (let i = 0; i < ass.length; i++) {
      ...
      if (asset && asset.gauge) {
        let pooledBalance = BigNumber(asset.gauge.userPoolBalance).times(
          asset.gauge.virtualPrice
        );
        let stakedBalance = BigNumber(asset.gauge.userGaugeBalance).times(
          asset.gauge.virtualPrice
        );
        let convexBalance = BigNumber(asset.convex.balance).times(
          asset.gauge.virtualPrice
        );
        ...
        <Grid item lg={12} md={12} sm={12} xs={12}>
          <Paper elevation={0} className={classes.itemWrapGrid}>
            <Grid container spacing={0}>
              ...
              >
                <Typography className={classes.title}>EXPD Balance:</Typography>
                <div className={classes.inline}>
                  <Typography className={classes.value}>
                    {formatCurrency(govToken ? govToken.balance : 0)}
                  </Typography>
                  <Typography className={classes.valueSymbol}>
                    {govToken ? govToken.symbol : ""}
                  </Typography>
                </div>
              </Grid>
            </Grid>
            ...
            <Grid container spacing={0}>
              ...
              >
                ...
                <div className={classes.inline}>
                  <Typography className={classes.value}>
                    {formatCurrency(
                      veToken && veToken.vestingInfo
                        ? veToken.vestingInfo.lockValue
                        : 0
                    )}
                  </Typography>
                  <Typography className={classes.valueSymbol}>
                    {veToken ? veToken.symbol : ""}
                  </Typography>
                </div>
              </Grid>
            </Grid>
